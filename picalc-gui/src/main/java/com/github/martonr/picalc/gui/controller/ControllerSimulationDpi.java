...
/**
 * Controller class for the DPI difference simulation view
 */
public final class ControllerSimulationDpi {
    /**
    ...
    /**
     * Text field for the quota value for the classical index calculation
     */
    @FXML
    private TextField quotaText;

    /**
     * If checked only scenarios where players don't reach the quota value will be simulated
     */
    @FXML
    ...
    /**
     * Button to save the simulation results to disk
     */
    @FXML
    ...
    /**
     * If checked limits the amount of simulations to the quota value
     */
    @FXML
     ...
     * specific vote count will have a positive, negative or no change to their power index when
     * using the DPI value instead of classical index values
     */
    @FXML
    private LineChart<Number, Number> shapleyProbabilityChart;
     ...
     */
    @FXML
    private LineChart<Number, Number> banzhafProbabilityChart;
     ...
     * the power index changes
     */
    @FXML
    private LineChart<Number, Number> stdevChart;
    ...
    private void startSimulation(ActionEvent event) {
        try {
            ...
            int quota, players, votes, simulations, count = 1;
            ...
            changeTextFieldLook(quotaText, null, "Quota", false);
    ...
    private void generateScenarioData(ActionEvent event) {
        try {
            // Parse the text field values
            int quota, players, votes, simulations, count = 1, setVote = 1;
            double epsilon = 0.001;
                ...
                try {
                    bw = Files.newBufferedWriter(Files
                            .createFile(Paths.get("./generated/" + year + "_" + month + "_" + day
                                    + "_" + hour + "_" + minute + "_dpisim_" + count + ".csv")));
                } catch (FileAlreadyExistsException ex) {
                ...
                try {
                    bw = Files.newBufferedWriter(Files.createFile(
                            Paths.get("./results/" + year + "_" + month + "_" + day + "_" + hour
                                    + "_" + minute + "_simulationdpi_result_" + count + ".csv")));
                } catch (FileAlreadyExistsException ex) {
                    // File with this name exists
                    count++;
                    // Too many files with the same name...
